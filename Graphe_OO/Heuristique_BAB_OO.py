...
#Fonctions de bornes :
def borne_1(graphe):
    deg_max = graphe.degre_graph().max()
    if(int(len(graphe.arretes)) == 0):
        return np.infty
    return int(np.ceil(deg_max / int(len(graphe.arretes))))

def borne_2(graphe, C = None):
    C = [] if C == None else C
    C = AlgoMatrice.algo_couplage(graphe)
    return len(C)/2

def borne_3(graphe):
    tmp = 2 * int(len(graphe.etiquettes)) - 1
    A = tmp - np.sqrt(tmp**2 - 8 * int(len(graphe.arretes)))
    return int(np.ceil(np.nan_to_num(A)/2))
...
def parcours_profondeur_NB_lite_OO(G):
    pile = []
    pile.append(Node(G,[]))
    best = len(G.etiquettes)*[9]
    total = len(G.etiquettes)
    ...
    while not (len(pile) == 0):
        ...
        #Recherche d'un arrête selon laquelle brancher
        e = graph.arretes[0] if len(graph.arretes)!=0 else None

        #MAJ de la pile si arréte il y a :
        if(e != None):
            pile.append(Node(graph.supprimer_sommet_copy(e[1]),C + [e[1]]))
            pile.append(Node(graph.supprimer_sommet_copy(e[0]),C + [e[0]]))
            continue
...
def parcours_profondeur_lite_OO(G,estimateurs_bornes=[borne_1,borne_2,borne_3]):
    pile = []
    pile.append(Node(G,[]))
    best = len(G.etiquettes)*[9]
    total = len(G.etiquettes)
    ...
    while not (len(pile) == 0):
        ...
        #Calcul d'une solution réalisable
        R = AlgoMatrice.algo_couplage(graph)
...
def parcours_profondeur_lite_ameliore_1_OO(G,estimateurs_bornes=[borne_1,borne_2,borne_3]):
    pile = []
    pile.append(Node(G,[]))
    best = len(G.etiquettes)*[9]
    total = len(G.etiquettes)
    ...
    while not (len(pile) == 0):
        ...
        if N.v != "":
            v = np.where(graph.adjacence[list(graph.etiquettes).index(N.v)]==1)[0]
            ve = [graph.etiquettes[i] for i in v]
            for e in ve:
                C.append(e)
                graph.supprimer_sommet(e)
        ...
        if(e != None):
            pile.append(Node(graph.supprimer_sommet_copy(e[1]),C + [e[1]]))
            pile.append(Node(graph.supprimer_sommet_copy(e[0]),C + [e[0]],e[1]))
...
#G1 = Graph(0, 0, file=r"test2.txt")
#G1 = Graph_A(0,0, file=r"test2.txt")
